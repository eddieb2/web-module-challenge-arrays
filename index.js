...
console.log(is31Flavors(testArr)); // false
...
function addFlavor(array, flavorToAdd) {
	return array.unshift(flavorToAdd);
}
...
Yonsole.log our function should remove a flavor from the end of the array and cthe resulting array.
...
console.log(originalFlavors.length); // 32
...
console.log(getFlavorByIndex(originalFlavors, 0)); // Rainbow Sherbert
console.log(getFlavorByIndex(originalFlavors, 1)); // Banana Nut Fudge
...
function getAverageWordLength(arr) {
	...
	// We need to count the total number of words in the array
	arr.forEach((item) => {
		// Use .split to separate each word of the flavor into individual arrays
		// Loop over each individual array and count the words
		item.split(' ').forEach((item) => {
			return (totalWords = totalWords + 1);
		});
	});

	// Divide the total number of words by the length of the array
	const avgWords = totalWords / arr.length;

	return avgWords;
}

// const testWordCount = ['hi', 'hi', 'hi', 'hi']
// console.log(getAverageWordLength(testWordCount));
